...
async function generateGiftLetterPdfBuffer(body) {
  ...
  const signatures = [{ caption: 'Donor Signature', name: (b.donorName && String(b.donorName).trim()) || undefined }].concat(
    (recips.length ? recips : [{}]).map(function (r) { return { caption: 'Recipient (Borrower) Signature', name: (r && r.name && String(r.name).trim()) || undefined }; })
  );

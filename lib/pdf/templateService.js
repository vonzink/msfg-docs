 ...
 * @param {object} [meta] - { name, category, icon, description }
 * @returns {Promise<object>} template config
 */
async function createTemplate(pdfBuffer, originalName, meta) {
  const id = crypto.randomUUID();
  const registry = readRegistry();
  const existingSlugs = registry.map(t => t.slug);
  ...
  // Update registry
  registry.push({ id, slug, name: config.name, category: config.category, icon: config.icon, fieldCount: fields.length });
...
function getTemplate(idOrSlug) {
  const registry = readRegistry();
  const entry = registry.find(t => t.id === idOrSlug || t.slug === idOrSlug);
...
function updateTemplate(idOrSlug, updates) {
  const config = getTemplate(idOrSlug);
  ...
  if (updates.name !== undefined) {
    const registry = readRegistry();
    const existingSlugs = registry.filter(t => t.id !== config.id).map(t => t.slug);
  ...
  if (idx !== -1) {
    registry[idx] = { id: config.id, slug: config.slug, name: config.name, category: config.category, icon: config.icon, fieldCount: config.fields.length };
    writeRegistry(registry);
  }
 ...
 */
async function fillTemplate(idOrSlug, formData) {
  const config = getTemplate(idOrSlug);
 ...
 */
function getTemplatePdfBytes(idOrSlug) {
  const config = getTemplate(idOrSlug);
  if (!config) return null;
  const pdfPath = path.join(TEMPLATES_DIR, config.id, 'template.pdf');

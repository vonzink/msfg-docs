...
const F = {
  ...
  // 5d Client info — IRS 4506-C "Client name, telephone, and address"
  clientName: 'form1[0].page_1[0].client_info[0].first_name[0]',
  clientPhone: 'form1[0].page_1[0].client_info[0].telephone_number[0]',
  clientStreet: 'form1[0].page_1[0].client_info[0].street_address[0]',
  clientCity: 'form1[0].page_1[0].client_info[0].city[0]',
  clientState: 'form1[0].page_1[0].client_info[0].state[0]',
  clientZip: 'form1[0].page_1[0].client_info[0].zip_code[0]',
 ...
 *        Override the IRS blank with an already-loaded PDF (e.g. an
 *        investor's pre-filled copy from dashboardSync.ensurePdfBytes).
 *        When provided, the worksheet stamp runs on top of that PDF;
 *        any fields the worksheet leaves blank stay as the investor
 *        pre-filled them (safeSetText early-returns on empty input).
 * @returns {Promise<Uint8Array>}
 */
async function generateForm4506cPdfBuffer(body, options) {
  const b = body || {};
  const opts = options || {};
  ...
  // 5a (IVES participant) + 5d (Client) are pre-filled in the investor's
  // PDF when one is selected — skip the third-party stamp so the
  // investor's blocks aren't overwritten by leftover worksheet input.
  // 5c "Unique identifier" stays (it's the borrower's loan number).
  if (!usingInvestorBase) {
    safeSetText(form, F.ivesName, b.f4506ThirdPartyName);
    safeSetText(form, F.ivesId, b.f4506IvesParticipantId);
    safeSetText(form, F.ivesSor, b.f4506IvesSorMailbox);
    // Address split: prefer explicit split-field overrides; fall back to
    // parseThirdPartyAddress() on the freeform "Address (full)" field.
    const tp = parseThirdPartyAddress(b.f4506ThirdPartyAddress);
    safeSetText(form, F.ivesStreet, tp.street || b.f4506ThirdPartyAddress);
    safeSetText(form, F.ivesCity, b.f4506IvesCity || tp.city);
    safeSetText(form, F.ivesState, (b.f4506IvesState || tp.state || '').toString().toUpperCase().slice(0, 2));
    safeSetText(form, F.ivesZip, b.f4506IvesZip || tp.zip);
    safeSetText(form, F.customerFile, b.f4506ThirdPartyCaf);

    // 5d Client (lender) — only stamped on the blank template path.
    safeSetText(form, F.clientName, b.f4506ClientName);
    safeSetText(form, F.clientPhone, b.f4506ClientPhone);
    safeSetText(form, F.clientStreet, b.f4506ClientStreet);
    safeSetText(form, F.clientCity, b.f4506ClientCity);
    safeSetText(form, F.clientState, (b.f4506ClientState || '').toString().toUpperCase().slice(0, 2));
    safeSetText(form, F.clientZip, b.f4506ClientZip);
  }

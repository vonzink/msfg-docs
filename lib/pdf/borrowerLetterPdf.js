...
async function generateBorrowerLetterPdfBuffer(letter) {
  ...
  /* pdf-lib's widthOfTextAtSize under-measures StandardFont strings that
     contain multiple spaces, so a multi-word fill-in value (kept as one
     token for a continuous underline) under-advances and swallows the
     following space/comma. Measure value tokens glyph-by-glyph, matching
     the per-glyph advance the PDF viewer actually uses. */
  function glyphWidth(text, font, size) {
    const str = sanitize(text);
    let w = 0;
    for (const ch of str) w += font.widthOfTextAtSize(ch, size);
    return w;
  }
  ...
  function layoutRuns(runs, maxW, size) {
    ...
    let w = 0;
    toks.forEach((t) => {
      const tw = t.value ? glyphWidth(t.text, serif, size) : widthOf(t.text, serif, size);
  ...
  function drawCertifyAndSignatures(certify, signatures) {
    ...
    // Signature line only — no separate DATE line. The letter's date is in
    // the masthead (top-right).
    const sigW = Math.round(contentW * 0.62);
    sigs.forEach((sig) => {
      const printedName = sig.name != null && String(sig.name).trim() !== '' ? String(sig.name).trim() : '';
      const rowH = printedName ? PX(56) : PX(46);
      needRoom(rowH);
      const lineY = y - PX(26);
      ...
      if (printedName) {
        // Printed name under the line, then the small role caption.
        drawText(left + PX(1), lineY - PX(13), printedName, { size: SIZE.value, font: serif, color: C.ink });
        drawTracked(left, lineY - PX(25), (sig.caption || 'Signature').toUpperCase(), {

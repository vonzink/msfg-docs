...
async function generateBorrowerLetterPdfBuffer(letter) {
  ...
  function drawCertifyAndSignatures(certify, signatures) {
    ...
    sigs.forEach((sig) => {
      ...
      needRoom(rowH);
      const lineY = y - PX(26);
      // Signature line is intentionally left blank — these documents are
      // printed for the borrower to sign by hand. (We do NOT pre-print the
      // borrower's name on the line.)

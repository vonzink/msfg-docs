  ...
  function drawCertifyAndSignatures(certify, signatures) {
    ...
    for (let i = 0; i < sigs.length; i += 2) {
      ...
      pair.forEach((sig, ci) => {
        ...
        const lineY = y - PX(24);
        if (sig.name) {
          drawText(cx + PX(2), lineY + PX(3), String(sig.name), { size: SIZE.value, font: sansBold, color: C.ink });
        }
        ...
        // Date sub-line under the caption.
        const dY = lineY - PX(30);
        page.drawLine({ start: { x: cx, y: dY }, end: { x: cx + colW * 0.5, y: dY }, thickness: 0.75, color: C.line });
        drawTracked(cx, dY - PX(10), 'DATE', {
          size: SIZE.sigCaption, font: monoBold, color: C.faint, tracking: SIZE.sigCaption * 0.14,
        });
      });
      y -= cellH + PX(14);

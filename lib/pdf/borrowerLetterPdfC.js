...
async function generateBorrowerLetterPdfBufferC(letter) {
  ...
  /* Value tokens are drawn bold and kept as one token (continuous underline);
     pdf-lib also under-measures multi-space StandardFont strings. Measure them
     glyph-by-glyph in the SAME font they're drawn in (serifBold) so the next
     run doesn't overlap the value. */
  ...
  function layoutRuns(runs, maxW, size) {
    ...
    let line = [], w = 0;
    toks.forEach((t) => {
      const tw = t.value ? glyphWidth(t.text, serifBold, size) : widthOf(t.text, serif, size);
  ...
  function drawCertifyAndSignatures(certify, signatures) {
    ...
    // Signature line only — no pre-printed borrower name, no DATE line
    // (the letter's date is in the masthead).
    const sigW = Math.round(contentW * 0.62);
    sigs.forEach((sig) => {
      const printedName = sig.name != null && String(sig.name).trim() !== '' ? String(sig.name).trim() : '';
      const rowH = printedName ? PX(60) : PX(50);
      needRoom(rowH);
      const lineY = y - PX(24);
      page.drawLine({ start: { x: left, y: lineY }, end: { x: left + sigW, y: lineY }, thickness: 1, color: C.ink });
      if (printedName) {
        drawText(left + PX(1), lineY - PX(12), printedName, { size: SIZE.body, font: serif, color: C.ink });
        drawTracked(left, lineY - PX(24), (sig.caption || 'Signature').toUpperCase(), {

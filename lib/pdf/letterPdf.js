...
async function generateLetterPdfBuffer(letter) {
  ...
  function drawSignatures() {
    ...
    // Pre-size the stamped signature (settings: "Signature Max Width", points)
    // so the row reserves room for it. Aspect ratio preserved; capped by the
    // signature-line width and a safety max height.
    let stampW = 0, stampH = 0;
    if (sigImage) {
      const maxW = (l.signatureImage && l.signatureImage.maxWidth > 0) ? l.signatureImage.maxWidth : 120;
      const s = Math.min(maxW / sigImage.width, colW / sigImage.width, 80 / sigImage.height);
      stampW = sigImage.width * s;
      stampH = sigImage.height * s;
    }
    const rowH = Math.max(44, Math.round(stampH) + 24);
